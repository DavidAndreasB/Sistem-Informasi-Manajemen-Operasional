...
@section('content')
        ...
        <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select id="inputMesin" class="form-control">
                                ...
                                @foreach($tarifs as $mesin => $harga)
                                    <option value="{{ $harga }}">{{ $mesin }} (Rp {{ number_format($harga, 0, ',', '.') }}/jam)
                                    </option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="col-lg-8">
                <div class="card shadow mb-4">
                    ...
                    {{-- Hidden Form untuk Export PDF --}}
                    <form id="pdfForm" action="{{ route('simulasi.pdf') }}" method="POST" target="_blank"
                        style="display: none;">
                        @csrf
                        <input type="hidden" name="items" id="pdfItems">
                        <input type="hidden" name="total" id="pdfTotal">
                    </form>
                    <div class="card-body">
    ...
    <script>
        let total = 0;
        let simulasiData = []; // Array untuk menyimpan data simulasi
        ...
        function hapusBaris(btn, subtotal) {
            ...
            let index = row.rowIndex - 1; // -1 karena ada header

            // Hapus dari array data
            simulasiData.splice(index, 1);
        ...
        function resetTabel() {
            ...
            total = 0;
            simulasiData = []; // Reset data
            updateTotal(0);
        }

        function exportPdf() {
            // Validasi: pastikan ada data simulasi
            if (simulasiData.length === 0) {
                alert("Tidak ada data simulasi untuk dicetak. Silakan tambahkan item terlebih dahulu.");
                return;
            }

            // Isi hidden form dengan data
            document.getElementById("pdfItems").value = JSON.stringify(simulasiData);
            document.getElementById("pdfTotal").value = total;
